    ...
    <div class="card">
        ...
        <div class="card-body">
          <form action="{{ route('sellers.update', $seller->id) }}" method="POST">
              ...
              <div class="form-group row">
                  <label class="col-sm-3 col-from-label" for="password">{{translate('Monthly Price')}}</label>
                  <div class="col-sm-3">
                      <input type="checkbox" style="width: 20px;" {{$seller->monthly_price ? "checked" : ""}} name="monthly_price" class="form-control">
                  </div>
              </div>
