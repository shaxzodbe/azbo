    			...
    			<div class="col-lg-12 table-responsive">
                    ...
                    @foreach ($order->orderDetails as $key => $orderDetail)
                      <tr>
                        ...
                        <td>
                          @if ($orderDetail->product != null)
                            <strong><a href="{{ route('product', $orderDetail->product->slug) }}" target="_blank" class="text-muted">{{ $orderDetail->product->getTranslation('name') }} @if($orderDetail->price == 0 && $orderDetail->product->bonus)  <span style="color: red">({{ translate('bonus') }}) </span> @endif</a></strong>
                            <small>{{ get_variant_string($orderDetail->variation) }}</small>
                          @else
