<div class="card border-0 shadow-sm rounded">
    <div class="card-header">
        ...
        <div class="text-right">
            <span class="badge badge-inline badge-primary">@php echo count(Session::get('cart')); @endphp {{translate('Items')}}</span>
        </div>
    </div>
        ...
        <table class="table">
            <thead>
                <tr>
                    ...
                    <th class="product-total text-right">@php echo count(Session::get('cart')); @endphp</th>
                </tr>
            </thead>
            <tbody>
            @php
                ...
                @foreach (Session::get('cart')->where('owner_id', Session::get('owner_id')) as $key => $cartItem)
                    ...
                    <tr class="cart_item">
                        <td class="product-name">
                            ...
                            @if($cartItem['bonus'])
                                <span style="color: #228B22	"> ( {{ translate('bonus') }} )</span>
                            @endif
...
{{--                <tr class="cart-subtotal">
                ...
                <tr class="cart-total">
                    ...
                    <td class="text-right">
                        <strong><span>{{--{{ single_price($total) }}--}} {{ $subtotals }}</span></strong>
                    </td>
                </tr>
            </tfoot>
        </table>
