            ...
            <div class="col-xxl-8 col-xl-10 mx-auto text-left">
                ...
                @php
                   ...
                   $cart = Session::get('cart');
                    ...
                    foreach ($cart as $key => $cartItem){
                        ...
                        if(\App\Product::find($cartItem['id'])->added_by == 'admin'){
                            ...
                            if(\App\BusinessSetting::where('type','bonus_product_activation')->first()->value) {
                                // check if admin products has bonus then add them ! 
                                $bonus_products = $cart->where('parent_id', $cartItem['id']);
                                foreach($bonus_products as $bonus) {
                                    array_push($admin_products, $bonus['id']);
                                }
                            }
                        }
                        else{
                            ...
                            if(\App\BusinessSetting::where('type', 'bonus_product_activation')->first()->value) {
                                $seller_products_tmp = $seller_products;
                                foreach($seller_products_tmp as $key => $p) {
                                    foreach($p as $s) {
                                        $item = $cart->where('parent_id', $s);
                                        foreach($item as $it) {
                                            array_push($seller_products[$key], $it['id']);
                                        }
                                    }
                                }
                            }
                    ...
                    // $cart = Session::get('cart');
                    //array_push($admin_products, $cart[$bonus_indice]['id']);
                                ...
                                <li class="list-group-item">
                                    <div class="d-flex">
                                        ...
                                        <span class="fs-14 opacity-60">{{ $product->getTranslation('name') }} 
                                            ...
                                            @if(\App\BusinessSetting::where('type', 'bonus_product_activation')->first()->value) 
                                                @php
                                                    $b = $cart->where('id', $cartItem)->first();
                                                @endphp 
                                                @if($b && $b['bonus'])
                                                    <span style="color: red;"> ({{ translate('bonus') }})</span>
                                                @endif
                                            @endif
                                        </span>
                                    </div>
