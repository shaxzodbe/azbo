                        ...
                        <div class="text-left">
                            ...
                            <div class="row align-items-center">
                                <div class="col-auto" style="width: 60%;">
                                    ...
                                    @endif
                                    <form method="post" action="{{ route('post_confirm_code') }}">
                                        <div class="form-group">
                                            <label for="exampleInputEmail1"><strong>Kodni tastiqlash:</strong></label>
                                            <input type="hidden" name="ref_id" value="{{session()->get('ref_id')}}">
                                            <input type="text" name="code" class="form-control border-gray-800" placeholder="kodni tasdiqlash...">
                                        </div>

                                        @csrf
                                        <button type="submit" class="btn btn-primary right">{{ translate('Message Seller')}}</button>
                                    </form>
                                </div>
                            ...
                            <div class="mt-3">
                                @if ($qty > 0)
                                    <a href="{{route('home')}}" class="btn btn-soft-primary mr-2 add-to-cart fw-600"
                                        ...
                                        <span class="d-none d-md-inline-block"> {{ __("Bosh sahifa")}}</span>
                                    </a>
                                    @if(Auth::check())

                                    <!-- && $detailedProduct->added_by == 'admin' -->
                                        @if(\App\BusinessSetting::where('type', 'monthly_payment')->first()->value == 1)
                                            <a style="color: white;" href="{{ route('intent_order_create',['product'=>$detailedProduct->id,'calculate'=>$calculate[0]['prices'][0]['duration']]) }}" type="button" class="btn btn-info fw-600"
                                               onclick="showCheckoutModal1()"> {{ __('Tastiqlash') }}</a>
                                        @endif
                                    @else
