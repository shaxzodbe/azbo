...
use App\Models\TransactionPayme;
...
class Transaction extends Database
{
    ...
    public function save()
    {
        ...
        if (!$this->id) {
            ...
            $transaction = TransactionPayme::create([
            ...
            $params['perform_time'] = $this->perform_time ? $this->perform_time : null;
            $params['cancel_time']  = $this->cancel_time ? $this->cancel_time : null;
            $params['state']       = 1 * $this->state;
            $params['reason']      = $this->reason ? 1 * $this->reason : null;
            $params['paycom_transaction_id']  = $this->paycom_transaction_id;
            $params['id']          = $this->id;

            $transaction = TransactionPayme::where([
                'paycom_transaction_id' => $this->paycom_transaction_id,
                'id' => $this->id
            ])->first();
    ...
    public function find($params)
    {
        ...
        if (isset($params['id'])) {
            $transaction = TransactionPayme::where('paycom_transaction_id', $params['id'])->first();
            ...
            $transaction = TransactionPayme::whereIn('state', [1, 2])->where('order_id', $params['account']['order_id'])->first();;
        } else {
