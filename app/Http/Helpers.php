...
if (!function_exists('sendSMS')) {
    ...
    {
        if(OtpConfiguration::where('type', 'broker_api')->first()->value == 1) {

            $username = config("otp.BROKER_API_USERNAME"); //put broker api username here
            $password = config("otp.BROKER_API_PASSWORD"); // put broker api password sid here
            ...
            $params = [
                "messages" => [
                    "recipient" => $to, // substr($to,0,1) === '+' ? substr($to,1): $to, // "998993966139"
                    "message-id" => 'azbo' . rand(100000, 999999),
                    "sms" => [
                        "originator" => "Azbo.uz",
                        "content" => [
                            "text" => $text
            ...
            // dd('api response: ' . $response, $params, $headers, $httpcode, $username, $password);
            return $response;
 ...
 * Send request to alifshop api 
 * 
 * 30/06/2021
 */
if (!function_exists('alifshop_api')) {
    function alifshop_api($params)
    {
        $token = "ysgcvaktcekaskycevwusacgWsagc"; 
        $url   = "https://gateway.alifshop.uz/api/partner/items/create_or_update";
...
if(!function_exists('get_variant_string')) {
    function get_variant_string($variant) {
        $variants = explode('-', $variant);
        $name = [];
        foreach($variants as $v) {
            $attr = \App\Attribute::find($v);
            if ($attr) {
