...
<?php endif; ?>
<head>
    <meta name="webmoney" content="A93350D0-E0EA-4D90-91F1-07595D9BD2E9"/>
    ...
    <?php if(!isset($detailedProduct) && !isset($customer_product) && !isset($shop) && !isset($page)): ?>
...
<body>
    ...
    <script>
        ...
        function updateNavCart(){
            $.post('<?php echo e(route('cart.nav_cart')); ?>', {_token: AIZ.data.csrf }, function(data){
                $('#cart_items').html(data);
            });
        }

        function removeFromCart(key){
            $.post('<?php echo e(route('cart.removeFromCart')); ?>', {_token: AIZ.data.csrf, key:key}, function(data){
                updateNavCart();
            ...
            if($('#option-choice-form input[name=quantity]').val() > 0 && checkAddToCartValidity()){
                $.ajax({
                   ...
                   success: function(data){
                       ...
                       /** installments */
                       var currency_symbol_input = document.getElementById('currency_symbol');

                        if(currency_symbol_input != null || currency_symbol_input != undefined) {
                            var installment_btn_active = $('.installments button.active');

                            var chosen_price = parseInt(data.price.replace(/ +/g, ""));
                            var percent = parseInt(installment_btn_active.data('percent'))
                            var period = parseInt(installment_btn_active.data('period'));
                            var profit = parseInt(installment_btn_active.data('profit'));


                            var percent_price = chosen_price * (percent / 100);
                            var profit_price = chosen_price * (profit / 100);
                            var price = (chosen_price + percent_price + profit_price)/period;

                            var final_price = Math.round(price);

                            $('#monthly_price').html(final_price.toLocaleString() + ' ' + currency_symbol_input.value);
                            $('#monthly_percent').html('(' + percent +'%)');
                        }
...
<?php /**PATH /var/www/azbo/resources/views/frontend/layouts/app.blade.php ENDPATH**/ ?>